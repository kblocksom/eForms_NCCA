...
verification <- subset(params[['userDataRMD']][['VERIFICATION']][[1]], 
                       PARAMETER %in% c('SITE_ID','VISIT_NO', "DATE_COL","LOC_NAME",
                                        "VALXSITE",'STATION_DEPTH','GEN_COM'))

verification_tbl <- subset(verification, !(PARAMETER %in% c("CREW")), select=c('PARAMETER','RESULT'))
...
profile_tbl.wide <- base::subset(profile_tbl.wide, select = c('DEPTH','DO','TEMPERATURE','PH','CONDUCTIVITY'))
