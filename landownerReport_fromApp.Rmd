...
verification <- dplyr::filter(params[['userDataRMD']][['VERIFICATION']][[1]], PARAMETER %in% c('SITE_ID','VISIT_NO', "DATE_COL","STATE","LOC_NAME","CREW",
                                                                                               "ACTUAL_LAT_DD","ACTUAL_LON_DD","VALXSITE",'STATION_DEPTH',
                                                                                               "GEN_COM","DRCTNS","CREW_LEADER")) %>%
...
habitat_tbl <- mutate(habitat, PARAMETER=dplyr::recode(PARAMETER, 'HABITAT'='Habitat Type',
                                                       'BOTTOM_TYPE'='Bottom type','MARINE_DEBRIS_TYPE'='Debris type present',
...
assessment <- dplyr::filter(params[['userDataRMD']][['ASSESSMENT']][[1]], PARAMETER %in% c('WEATHER','DOM_LAND_USE','FOR_AGE_CLASS',
  ...
  mutate(PARAMETER = recode(PARAMETER,"DOM_LAND_USE"='Shoreline dominant land use', 
                            ...
                            'TRASH_OTHER_PLASTICS'='Other plastic trash','TRASH_OTHER'='Other trash','TRASH_TIRES'='Tires',
...
write.csv(samples_tbl,"c:/users/kblockso/local/TestSamplesTable.csv")
